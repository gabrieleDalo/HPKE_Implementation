...
{   
    ...
    for (int i = 1; i <=4; i++){
        for(int j = 1; j <= n_repeat; j++){
            ...
            const unsigned char *pt = bytes_from_hex(get_pt -> valuestring, &output_length);
            ...
            const unsigned char *ct = bytes_from_hex(get_ct -> valuestring, &output_length);

            cJSON *get_info = cJSON_GetObjectItem(test, "info");
            if(get_info == NULL || !cJSON_IsString(get_info)){
                get_info = cJSON_CreateString("");
            }
            const unsigned char *info = bytes_from_hex(get_info -> valuestring, &output_length);
            ...
            const unsigned char *aad = bytes_from_hex(get_aad -> valuestring, &output_length);
            ...
            const unsigned char *enc_key = bytes_from_hex(get_enc -> valuestring, &output_length);
