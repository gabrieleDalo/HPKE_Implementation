...
#define CONSTSIZE 128

void store_string(const char *filepath, const char *data, size_t data_len)
{
...
{   
    ...
    printf("Inserisci l'id della modalità da testare (numero da 0 a 3): ");
    check = scanf("%d", &id_mode);
    
    if (check != 1 || id_mode < 0 || id_mode > 3) {
        printf("Input non valido. Assicurati di inserire un numero corretto\n");
        return 1;
    }

    id_mode++;
    char i_value[10];
    sprintf(i_value, "%d", id_mode);
    ...
    unsigned char pks[CONSTSIZE] = {0};    // Sender public key
    size_t pkslen = sizeof(pks);

    EVP_PKEY *skr = NULL;          // Receiver private key
    unsigned char pkr[CONSTSIZE] = {0};    // Receiver public key
    size_t pkrlen = sizeof(pkr);

    unsigned char enc[CONSTSIZE] = {0};    // Encapsulated public sender value
    size_t enclen = sizeof(enc);

    unsigned char ciphertext[CONSTSIZE] = {0};
    size_t ciphertext_len = sizeof(ciphertext);
    unsigned char plaintext[CONSTSIZE] = {0};
    ...
    if(strncmp((char *) enc_key, (char *) enc, enclen) != 0){      // Check if the enc generated is the same present in the test_vectors
    ...
    if(strcmp((char *) ct, (char *) ciphertext) != 0){    // Check if the ciphertext generated is the same present in the test_vectors
        printf("Different ciphertext\n");
    }

    id_mode--;
    char value[10];
    sprintf(value, "%d", id_mode);
    char enc_name[10] = "enc";
    strcat(enc_name, value);
    strcat(enc_name, ".bin");

    char cipher_name[20] = "cipher";
    strcat(cipher_name, value);
    strcat(cipher_name, ".bin");

    char pkr_name[10] = "pkr";
    strcat(pkr_name, value);
    strcat(pkr_name, ".bin");

    store_string(enc_name, (char *) enc, enclen);
    store_string(cipher_name, (char *) ciphertext, ciphertext_len);
    store_string(pkr_name, (char *) pkr, pkrlen);
    ...
    if (OSSL_HPKE_decap(rctx, enc_key, sizeof(enc_key), skr, info, info_len) != 1){
        printf("Error in decap\n");
        return 1;
    }

    if (OSSL_HPKE_open(rctx, plaintext, &plaintext_len, aad, aad_len, ct, sizeof(ct)) != 1){
    ...
    if(strcmp((char *) pt, (char *)plaintext) != 0){
        ...
        printf("Interoperability verified\n");
    }
